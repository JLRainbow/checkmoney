...
public class PagePlugin implements Interceptor {
	...
	/**
	   *   select
		 ...
		 * sum(ddd) ss,
		 * 	articleName,
	     *  (SELECT loginName from t_userinfo u where u.id=userId) loginName,
		 * 	(SELECT userName from t_userinfo u where u.id=userId) userName,
		 * sum(ddd) ss
		 * from article	
		 * 兼容以上子查询
	...
	public static void main(String[] args) {
		String sql="  select "+
		 "	articleNo "+
		 " from article left jion aefv where 1=(SELECT userName from t_userinfo u where u.id=userId) "
	...
	public Map<String, Object> toHashMap(Object parameterObject) {
		...
		try {
			...
			if (flag) {  
				...
				froMmap.put("t_table", table.tableName());
			}else{

...
(function() {
	lyGrid = (function(params) {
		...
		var cHeadTable = function(divid) {
			...
			th.setAttribute("style", "text-align:center;width: 35px;vertical-align: middle;display: " + cn + ";");
			tr.appendChild(th);
			var cbk = "";
			if (!conf.checkbox) {
				cbk = "none";
		...
		var cBodytb = function(divId,jsonData){
			...
			for(;d<e;d++){
				...
				if(CommnUtil.notNull(rowdata)){
					...
					ntd_d.setAttribute("style", "text-align:center;width: 35px;display: " + cn + ";");
					...
					$.each(column, function(o) {
						if (!column[o].hide || column[o].hide == undefined) {
							...
							td_o.setAttribute("style", "text-align:" + column[o].align + ";width: " + column[o].width + ";vertical-align: middle;overflow: hidden;text-overflow: ellipsis;");
		...
		var fenyeDiv = function(divid, jsonData) {
			...
			bnav.className = "list-page";
			bdiv.appendChild(bnav);
			var bp = document.createElement("p");
			bp.className = "page-info";
			bnav.appendChild(bp);
			bp.innerHTML = '总&nbsp;' + totalRecords + '&nbsp;条&nbsp;&nbsp;每页&nbsp;' 
	        + conf.pageSize + '&nbsp;条&nbsp;&nbsp;共&nbsp;' + totalPages + '&nbsp;页';
			//隐藏原有的
			/*var bdiv = document.createElement("div");
			bdiv.setAttribute("style", "vertical-align: middle;");
			...
			divul_2.className = "pager";
			bnav.appendChild(divul_2);
		...
		Array.prototype.t_each = function(f) { // 数组的遍历
		...
		var lyGridUp = function(jsonUrl) { // 上移所选行
			...
			;
			arr.reverse().t_each(function(tr) {
				var ck = getChkBox(tr);
				if (ck.checked) {
